...
float LevelManager::GetTimeLimit() { return mBaseTimeLimit * mTimeModifier; }

Vector2f LevelManager::GetStartPosition() { return mStartPosition; }
...
int ** LevelManager::NextLevel(VertexArray & rVaLevel)
{
	...
	string s;
	while (getline(inputFile, s)) { ++mLevelSize.y; }
	mLevelSize.x = s.length();
	inputFile.clear();
	inputFile.seekg(0, ios::beg);
	int** arrayLevel = new int*[mLevelSize.y];
	for (int i = 0; i < mLevelSize.y; ++i) { arrayLevel[i] = new int[mLevelSize.x]; }
	string row;
	int y = 0;
...
Vector2i LevelManager::GetLevelSize() { return mLevelSize; }

int LevelManager::GetCurrentLevel() { return mCurrentLevel; }